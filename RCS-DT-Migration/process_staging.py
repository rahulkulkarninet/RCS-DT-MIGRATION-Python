...
# pandas holds datetimes as datetime64[ns], which cannot represent anything
# outside 1677-09-21..2262-04-11. SQL Server's date reaches year 9999 and its
# datetime reaches 1753-9999, so a source value such as RC_DRDBINVOICE's
# Invoice_Date '2503-10-02' parsed with errors='coerce' silently became NULL:
# the row loaded, the date did not, and nothing was logged. A year that far out
# is usually bad source data, but that call belongs to whoever reads the
# staging table, not to this layer - so the value is preserved and reported.
PANDAS_MIN_YEAR = 1678
PANDAS_MAX_YEAR = 2261

# Placeholder year for the round trip below. Must be a leap year or 29 February
# would not survive the substitution.
_PLACEHOLDER_YEAR = 2000

# A standalone four-digit run, i.e. the year in every format seen in these
# extracts ('2503-10-02', '2/10/2503', '2503-10-02 09:15:00').
...
class StagingProcessor:
    ...
    def verify_staging_tables_exist(self) -> List[str]:
        """Configured staging tables that are absent from the database.
