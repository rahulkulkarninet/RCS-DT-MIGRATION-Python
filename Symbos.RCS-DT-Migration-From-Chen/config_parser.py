...
class ConfigParser:
    ...
    def _resolve_lookups(self, config: Dict[str, Any]):
        ...
        for var_name, var_config in variables.items():
            try:
                filter_col = var_config.get('filter_col')
                filter_value = var_config.get('filter_value')

                if var_config.get('filter_normalized'):

                    col_expr = (f"LOWER(REPLACE(REPLACE(REPLACE("
                                f"[{filter_col}], ' ', ''), CHAR(9), ''), '-', ''))")
                    needle = ''.join(str(filter_value).lower().split())
                    needle = needle.replace('-', '').replace("'", "''")
                    where = f"WHERE {col_expr} = '{needle}'"
                else:
                    where = f"WHERE [{filter_col}] = '{filter_value}'"
                ...
                result = self.db_helper.execute_query(query)
                if len(result) > 1:
                    # Normalising widens the match, so say so rather than silently
                    # taking whichever row came back first.
                    self.logger.warning(
                        f"{var_name}: {len(result)} rows matched "
                        f"{var_config.get('main_table')}.{filter_col} = "
                        f"{filter_value!r}; using the first")
