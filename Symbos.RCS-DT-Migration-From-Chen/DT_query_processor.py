...
PRINT 'All files executed successfully within single transaction.';"""
    ...
    def _scan_exported_sql_files(self, export_path: Path, sequence_range: Tuple[int, int] = None) -> OrderedDict:
        ...
        if sequences:
            ...
            # Report duplicate sequences with the filenames involved. Only the
            # metafield fan-out produces them now - the numbered files on disk are
            # one per sequence since 35.Update_tblAccount_name became 77 - but the
    ...
    ALWAYS_RERUN_TABLES: frozenset = frozenset()
        ...
        try:
            ...
            try:
                for i, (key, file_info) in enumerate(sql_files.items(), 1):
                    ...
                    rebuilds_truncated_table = (
                        str(file_info.get('table_name', '')).upper()
                        in self.ALWAYS_RERUN_TABLES
                    )
                    if filename in already_done and rebuilds_truncated_table:
                        self.logger.info(
                            f"[{i}/{len(sql_files)}] re-running {filename} despite the "
                            f"checkpoint: it rebuilds a staging table that this run's "
                            f"truncate emptied"
                        )
                    if filename in already_done and not rebuilds_truncated_table:
    ...
    def generate_metrics_query(self, metric_key: str, config: dict, load_id: int, session_id: int) -> str:
        ...
        try:
            ...
            if filter_by == 'none':
                # A per-customer staging table, which holds this run's rows and only
                # this run's rows because the staging step truncates it on the way in.
                # RC_STAGING_NOTES_ACCOUNT is the case this exists for: it carries no
                # LoadID, for the same reason the extracts it is folded from carry none.
                pass
            elif filter_by == 'loadid':
