...
CTE_MISSData AS(
    ...
    CROSS APPLY (VALUES
        ('Amount_Paid_Total_Payments', NULL,NULL,Amount_Paid_Total_Payments,NULL),
        ('Date_Payment_Made', NULL,NULL,NULL,Date_Payment_Made),
        ('Receipt_No',Receipt_No, NULL,NULL,NULL),
        ('DI_Payment_Method',DI_Payment_Method, NULL,NULL,NULL),
        ('Payplan_Status_Receipt_Details',Payplan_Status_Receipt_Details, NULL,NULL,NULL),
        ('Customer_Contact_Details',Customer_Contact_Details, NULL,NULL,NULL)
...
INSERT  INTO tblMetaValue_AccountSpecifics WITH ( ROWLOCK )
    ...
    1
    FROM    CTE_MISSData
            INNER JOIN Metafield_accountspecifics_details Meta on Meta.Name = CTE_MISSData.Name
            INNER JOIN tblAccount A ON A.AccountNumberPrevious = CONVERT(nvarchar(100), CTE_MISSData.Full_Debt_Code)
