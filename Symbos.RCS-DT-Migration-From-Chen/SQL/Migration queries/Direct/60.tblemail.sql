...
INSERT  INTO tblEmail WITH ( ROWLOCK )
        ...
        FROM    RC_EMAIL_EXTRACT E
                ...
                INNER JOIN tblCommunication Comm WITH ( NOLOCK ) ON Comm.Z_REF = CONVERT(nvarchar(200), E.Doc_Hist_Code)
                                                           AND Comm.LoadID = {{LoadID}}
                OUTER APPLY ( SELECT TOP 1
                                        C.ContactID
                              FROM      tblContact C WITH ( NOLOCK )
                              WHERE     C.UserName = LTRIM(RTRIM(REPLACE(E.Op_code, NCHAR(160), ' ')))
	...
	FROM RC_EMAIL_EXTRACT RCE
                join tblaccount A on A.AccountNumberPrevious = CONVERT(nvarchar(100), RCE.Extended_Debt_Code)
                join tblcontact C on C.Z_REF2 = RCE.Debtor_Code
                                                and C.Z_REF = CONVERT(nvarchar(200), RCE.Extended_Debt_Code)
                                                and C.LoadID = {{LoadID}}
                join tblEmail E on E.Z_REF = CONVERT(nvarchar(200), RCE.ZID)
                                         and E.AccountID = A.AccountID
                                         and E.Z_IDSTR = RCE.Debtor_Code
                                         and E.CreateSessionID = {{CurrentSessionID}}
		JOIN RC_DOCHIST_EXTRACT DH ON CONVERT(nvarchar(100), RCE.DOC_HIST_CODE) = DH.ZID
    WHERE A.LoadID = {{LoadID}}
          AND NOT EXISTS (
                        SELECT 1
