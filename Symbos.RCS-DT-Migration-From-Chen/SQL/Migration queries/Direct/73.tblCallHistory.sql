INSERT  INTO tblCallHistory
        ...
        FROM    RC_DEBT_CONTACTS DC
                INNER JOIN tblAccount A WITH ( NOLOCK ) ON A.AccountNumberPrevious = CONVERT(nvarchar(100), DC.Extended_Debt_Code)
        ...
        INSERT INTO tblCorrespondenceHistory  WITH ( ROWLOCK )
			...
			SELECT CH.CallHistoryID ,             -- CorrespondenceID
				...
				from RC_DEBT_CONTACTS DC
					join tblaccount A on A.AccountNumberPrevious = CONVERT(nvarchar(100), DC.Extended_Debt_Code)
                                        join tblcontact C on C.Z_REF2 = DC.Multi_Recipient_Key
                                                                        and C.Z_REF = CONVERT(nvarchar(200), DC.Extended_Debt_Code)
                                                                        and C.LoadID = {{LoadID}}
