INSERT  INTO tblLetter WITH ( ROWLOCK )
        ...
        FROM    RC_DOCHIST_EXTRACT D
                ...
                INNER JOIN tblCommunication Comm WITH ( NOLOCK ) ON Comm.Z_REF = CONVERT(nvarchar(200), D.Note_Key)
                INNER JOIN tblDocument Doc WITH ( NOLOCK ) ON Doc.Z_IDSTR = CONVERT(nvarchar(200), D.Note_Key)
...
INSERT INTO tblCorrespondenceHistory  WITH ( ROWLOCK )
		...
		FROM RC_DOCHIST_EXTRACT DH
				join tblaccount A on A.AccountNumberPrevious = CONVERT(nvarchar(100), DH.Extended_Debt_Code)
