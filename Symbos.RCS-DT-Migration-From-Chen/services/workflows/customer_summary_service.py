...
class CustomerSummaryService:
    ...
    def _archive_line(self, result: Dict[str, Any], migration_ok: bool) -> str:
        ...
        schema = result.get('archive_schema') or 'the archive schema'
        ...
        archived = sum(1 for t in tables.values() if not t.get('skipped'))
        rows = result.get('archive_rows_copied', 0)
        return f'{archived} table(s), {rows:,} row(s) renamed into {schema}'
