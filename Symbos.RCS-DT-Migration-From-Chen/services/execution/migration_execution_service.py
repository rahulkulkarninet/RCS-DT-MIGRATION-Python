...
# The one file that runs through sqlcmd: Loop/78.final loops and sps to run.sql.
# Named rather than inlined because the sequence moves whenever a file is inserted
# into the numbered flow, and a stale literal here silently routes the last direct
# file to sqlcmd (or the loop file to PyODBC) instead of failing.
SQLCMD_SEQUENCE = 78
PYODBC_MAX_SEQUENCE = SQLCMD_SEQUENCE - 1
    ...
    def execute_mixed_sql_methods_internal(
        ...
        try:
            ...
            for key, file_info in sql_files.items():
                ...
                if 1 <= sequence <= PYODBC_MAX_SEQUENCE:
                    pyodbc_files[key] = file_info
                elif sequence == SQLCMD_SEQUENCE:
                    sqlcmd_files[key] = file_info
                else:
                    customer_logger.warning(
                        f"File sequence {sequence} outside expected range (1-{SQLCMD_SEQUENCE}): {file_info.get('filename', 'unknown')}"
                    )

            customer_logger.info(
                f"Split files: {len(pyodbc_files)} for PyODBC (1-{PYODBC_MAX_SEQUENCE}), "
                f"{len(sqlcmd_files)} for SQLCMD ({SQLCMD_SEQUENCE})"
            )
            ...
            # Phase 1: Execute files 1-77 using PyODBC with transaction control.
            if pyodbc_files:
                customer_logger.info(
                    f"Phase 1: Executing {len(pyodbc_files)} files (sequences 1-{PYODBC_MAX_SEQUENCE}) using PyODBC..."
                )
                ...
                for key, result in pyodbc_results.items():
                    ...
                    elif result.get('skipped', False) and not result.get('error'):
                        # Already committed by the --resume-load-id run being resumed.
                        combined_results['method_breakdown']['pyodbc']['success'] += 1
                    else:
                ...
                if not pyodbc_success:
                    customer_logger.error(f"PyODBC phase (files 1-{PYODBC_MAX_SEQUENCE}) failed - stopping execution")
                    failed_pyodbc = [
            ...
            # Phase 2: Execute file 78 using SQLCMD.
            if sqlcmd_files:
                customer_logger.info(
                    f"Phase 2: Executing {len(sqlcmd_files)} files (sequence {SQLCMD_SEQUENCE}) using SQLCMD..."
                )
            ...
            customer_logger.info(
                f"  PyODBC (1-{PYODBC_MAX_SEQUENCE}): {combined_results['method_breakdown']['pyodbc']['success']} success, "
                f"{combined_results['method_breakdown']['pyodbc']['failed']} failed"
            )
            customer_logger.info(
                f"  SQLCMD ({SQLCMD_SEQUENCE}): {combined_results['method_breakdown']['sqlcmd']['success']} success, "
