    ...
    def process_staging_to_migration(self, processor: Any, validate_only: bool = False) -> Dict[str, Dict]:
        ...
        for i, customer_code in enumerate(valid_customers, 1):
            ...
            try:
                ...
                # Final step: rename the staging tables into the archive schema,
                # while they still hold this customer's extract. Nothing is
                # racing the next customer's truncate any more - the rename
                # rebuilds an empty staging table in the same transaction, so
                # the tables are back before this returns. Only worth doing, and
                # only trustworthy, when the migration itself succeeded.
