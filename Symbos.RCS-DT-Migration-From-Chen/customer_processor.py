...
from services.workflows.customer_cost_workflow_service import (
...
from services.workflows.customer_operator_contact_workflow_service import (
    CustomerOperatorContactWorkflowService,
)
...
from operator_contact_service import OperatorContactService
...
class CustomerProcessor:
    ...
    def __init__(self, environment: str, approval_policy: Optional[ApprovalPolicy] = None,
        ...
        self.operator_contact_service: Optional[OperatorContactService] = None
        ...
        self.customer_complainant_workflow_service = (
            CustomerComplainantWorkflowService()
        )
        self.customer_complaint_root_workflow_service = (
            CustomerComplaintRootWorkflowService()
        )
        self.customer_cost_workflow_service = (
            CustomerCostWorkflowService()
        )
        ...
        self.customer_operator_contact_workflow_service = (
            CustomerOperatorContactWorkflowService()
        )
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.cost_service = CostService(self.shared_db_helper)
            ...
            self.operator_contact_service = OperatorContactService(self.shared_db_helper)
    ...
    def archive_staging_tables(self, customer_code: str, db: str,
        ...
        """Final step: rename this customer's staging tables into the archive.

        Runs on this run's own connection. The archive step used to open a
        second one, to the separate archive database, and copy every row across
        it - which on Azure meant each row leaving the server and coming back.
        It renames the tables in place now, so there is no second database in
        the picture and nothing to connect to. tools/sweep_archive.py moves them
        on to ARCHIVE_DATABASE afterwards, outside the run.
        """
    ...
    def update_frequencies(
        ...
        using variables/frequency_codes.json, so 78.final loops and sps to run.sql can
    ...
    def update_costs(
        ...
        """
        Rebuild RC_STAGING_COSTS from RC_COSTS_EXTRACT using variables/cost_codes.json,
        resolving each configured cost code to its tblCostType and tblMasterCost row.
        Unlike the mapping workflows this writes a staging table for 51.tblcosts.sql
        rather than rewriting extract values in place.
        """
        return self.customer_cost_workflow_service.update_costs(
            self,
            customer_logger,
        )

    def _check_costs_step(
        self,
        customer_code: str,
        db: str,
        customer_logger: logging.Logger,
        ...
        """Check cost codes and report charged codes that cost_codes.json does not map."""
        return self.customer_cost_workflow_service.check_costs_step(
            self,
            customer_code,
            db,
            customer_logger,
    ...
    def update_operator_contacts(
        ...
        """
        Rewrite the operator columns of the extracts in place to the canonical
        tblContact.UserName, using variables/operator_contact_codes.json, so files 01,
        49, 58, 60, 64, 65 and 67 can read tblContact directly instead of joining the
        customer-specific CSRC_OperatorContactMapping. Same mechanism as the MA_Status
        and Payment_Method rewrites.
        """
        return self.customer_operator_contact_workflow_service.update_operator_contacts(
            self,
            customer_logger,
        )

    def _check_operator_contacts_step(
        self,
        customer_code: str,
        db: str,
        customer_logger: logging.Logger,
        ...
        """Check operator codes and report the ones the JSON does not map."""
        return self.customer_operator_contact_workflow_service.check_operator_contacts_step(
            self,
            customer_code,
            db,
            customer_logger,
    ...
    def _execute_mixed_sql_methods_internal(self, export_folder_path: str, customer_logger: logging.Logger) -> MixedExecutionResult:
        ...
        Files 1-77: PyODBC with transaction control
        File 78: SQLCMD for compatibility
