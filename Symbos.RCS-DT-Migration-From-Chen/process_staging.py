...
from notes_consolidation import NotesConsolidator
...
ADDITIONAL_REQUIRED_TABLES: Tuple[str, ...] = ('RC_STAGING_COSTS',
                                               'RC_STAGING_NOTES_ACCOUNT')


DERIVED_TABLES_TO_TRUNCATE: Tuple[str, ...] = ('RC_STAGING_NOTES_ACCOUNT',
                                               'RC_STAGING_COSTS')

# The extract whose load also builds the consolidated per-account notes.
NOTES_EXTRACT_TABLE = 'RC_NOTES_EXTRACT'
NOTES_ACCOUNT_TABLE = 'RC_STAGING_NOTES_ACCOUNT'

# NotesConsolidator emits the lowercase names the cleaned chunks use; the bcp sink
# reflects the real table and matches on the column names it finds there.
NOTES_ACCOUNT_COLUMNS: Dict[str, str] = {
    'extended_debt_code': 'Extended_Debt_Code',
    'entry': 'Entry',
...
PANDAS_MIN_YEAR = 1678
PANDAS_MAX_YEAR = 2261
...
def parse_date_series(series: pd.Series, dayfirst: bool = True) -> Tuple[pd.Series, List[str]]:
    """Parse a column of date strings, keeping years pandas cannot represent.

    Returns the parsed series and the list of distinct values that could not be
    parsed at all, so the caller can report them instead of dropping them silently.

    Values outside the pandas window are recovered by swapping the year for an
    in-range placeholder and handing the string back to pandas, so the format
    decision - ISO versus day-first, separators, any time part - stays exactly
    the one the main path would have made. Doing the fallback with dateutil was
    tried and rejected: with dayfirst=True it reads '2503-10-02' as 10 February,
    silently swapping day and month on ISO input.

    A recovered value forces the whole column to object dtype holding Python
    datetimes. Both sinks already handle that - the bcp writer formats date and
    datetime objects explicitly, and bulk_insert_chunked casts to object before
    handing rows to pyodbc.
    """
    parsed = pd.to_datetime(series, errors='coerce', format='mixed', dayfirst=dayfirst)

    # Non-empty in the source but NaT after parsing: either out of range or junk.
    source = series.astype(object)
    populated = source.notna() & (source.astype(str).str.strip() != '')
    failed = populated & parsed.isna()
    if not failed.any():
        return parsed, []

    # Positional, not label based: a chunked read can hand this a non-unique
    # index, and .at on a duplicate label returns a Series rather than a value.
    recovered: Dict[int, datetime] = {}
    unparsed: List[str] = []
    for position in failed.to_numpy().nonzero()[0]:
        raw = str(series.iloc[position]).strip()
        value = _parse_out_of_range(raw, dayfirst)
        if value is None:
            unparsed.append(raw)
        else:
            recovered[int(position)] = value

    if not recovered:
        return parsed, unparsed

    parsed = parsed.astype(object).where(parsed.notna(), None)
    for position, value in recovered.items():
        parsed.iloc[position] = value
    return parsed, unparsed


def _parse_out_of_range(raw: str, dayfirst: bool) -> Optional[datetime]:
    """Parse one value whose year is outside the pandas representable window."""
    years = _FOUR_DIGIT_YEAR.findall(raw)
    # Exactly one candidate, or which number is the year is guesswork.
    if len(set(years)) != 1:
        return None

    year = int(years[0])
    if PANDAS_MIN_YEAR <= year <= PANDAS_MAX_YEAR:
        return None      # in range already, so pandas failed for some other reason

    stand_in = pd.to_datetime(
        raw.replace(years[0], str(_PLACEHOLDER_YEAR), 1),
        errors='coerce', format='mixed', dayfirst=dayfirst,
    )
    if pd.isna(stand_in):
        return None
    try:
        return stand_in.to_pydatetime().replace(year=year)
    except ValueError:
        return None      # e.g. 29 February in a real year that is not a leap year
...
class StagingProcessor:
    ...
    def __init__(self, environment: str, shared_db_helper=None, shared_config_parser=None ):
        ...
        # table name -> the SQL/Schemas file that rebuilds it, read once at
        # startup by verify_rebuild_schema_files and used by the archive step.
        self.staging_schema_files: Dict[str, Any] = {}
    ...
    def initialize(self):
        ...
        try:
            ...
            # Fail here rather than mid-truncate on the first customer.
            missing_tables = self.verify_staging_tables_exist()
            if missing_tables:
                self.logger.error(
                    f"{len(missing_tables)} staging table(s) missing from "
                    f"{self.environment}: {', '.join(missing_tables)}. Deploy the "
                    f"matching schema from SQL/Schemas before running the migration."
                )
                return False

            # And here rather than after a table has been renamed away.
            schema_file_problem = self.verify_rebuild_schema_files()
            if schema_file_problem:
                self.logger.error(schema_file_problem)
                return False
    ...
    def truncate_staging_tables(self) -> bool:
        ...
        try:
            ...
            # Truncate each staging table. A failure has to leave this block by
            # raising: returning from inside the with exits the context manager
            # normally, which commits the truncates that already succeeded and
            # leaves staging half-emptied - the tables before the failure wiped,
            # the ones after it still holding the previous customer's extract.
            with self.db_helper.sqlalchemy_engine.begin() as conn:
                ...
                # Derived tables go in the same transaction as the extract tables they
                # are derived from, so staging cannot end up holding this customer's
                # extract next to the previous customer's derived rows.
                for table_name in DERIVED_TABLES_TO_TRUNCATE:
                    try:
                        conn.execute(text(f'TRUNCATE TABLE {table_name}'))
                    except Exception as e:
                        self.logger.error(f"Failed to truncate table {table_name}: {e}")
            ...
            self.logger.error(
                f"Error truncating staging tables (rolled back, nothing was "
                f"emptied): {e}"
            )
            return False

    def verify_staging_tables_exist(self) -> List[str]:
        """Required staging tables that are absent from the database.

        Every configured table is truncated at the start of each customer, so a
        missing one fails the run - but it used to fail per customer, part way
        through the truncate loop, after the migration session and LoadID had
        already been created. Asking once at startup reports every missing table
        up front instead.

        Covers ADDITIONAL_REQUIRED_TABLES as well as the keyword tables, so a
        staging table that no CSV loads is caught here rather than by the step
        that writes to it.
        """
        if not self.db_helper or not self.db_helper.sqlalchemy_engine:
            self.logger.error("Database connection not available")
        ...
        table_keywords = self.config.table_keywords if self.config else None
        configured = [name for name in (table_keywords or {}).values() if name]
        ...
        required = configured + [
            name for name in ADDITIONAL_REQUIRED_TABLES if name not in configured
        ]

        missing = []
        with self.db_helper.sqlalchemy_engine.connect() as conn:
            for table_name in required:
                # OBJECT_ID resolves a bare name against the default schema, the
                # same way TRUNCATE TABLE does, so this checks exactly what the
                # truncate will hit.
    ...
    def verify_rebuild_schema_files(self) -> str:
        """Why the staging tables could not be rebuilt after archiving, or ''.

        The archive step renames each staging table into the archive schema and
        rebuilds an empty one from that table's file in SQL/Schemas - the files
        rather than the table's own catalog entry, because only they carry the
        join indexes the migration queries depend on.

        A file missing from the set therefore does not fail the archive, it
        fails the *next* customer: the table has already been renamed away by
        the time anyone finds out, at the end of an otherwise successful run.
        Reading them here costs a few milliseconds once, and turns that into a
        message before the first customer starts.
        """
        configured = [name for name in self.config.table_keywords.values() if name]
        if not configured:
            return ''

        try:
            self.staging_schema_files = load_staging_schema_files(configured)
        except SchemaSetError as e:
            return (f'Cannot rebuild the staging tables after archiving them. '
                    f'{e}')
        except Exception as e:
            return (f'Cannot read the staging schema files: '
                    f'{type(e).__name__}: {e}')

        self.logger.info(
            f'Rebuild schema files resolved for {len(self.staging_schema_files)} '
            f'staging table(s)')
        return ''
    ...
    def clean_frame(self, frame: pd.DataFrame, db_columns: Dict[str, Dict]) -> pd.DataFrame:
        ...
        truncation_warnings = []
        unparsed_dates: Dict[str, List[str]] = {}
        ...
        for col in frame.columns:
            ...
            try:
                ...
                if 'int' in db_type or 'tinyint' in db_type or 'smallint' in db_type or 'bigint' in db_type:
                    ...
                    parsed, unparsed = parse_date_series(frame[col])
                    frame[col] = parsed
                    unparsed_dates[col] = unparsed
                elif 'date' in db_type:
                    parsed, unparsed = parse_date_series(frame[col])
                    unparsed_dates[col] = unparsed
                    # parse_date_series returns object dtype once anything was
                    # recovered, and .dt is only there for datetime64 columns.
                    if pd.api.types.is_datetime64_any_dtype(parsed):
                        frame[col] = parsed.dt.date
                    else:
                        frame[col] = parsed.map(
                            lambda v: v.date() if isinstance(v, datetime) else v)
        ...
        for col, values in unparsed_dates.items():
            if values:
                distinct = sorted(set(values))
                self.logger.warning(
                    f'Column {col}: {len(values)} value(s) stored as NULL, not a '
                    f'recognisable date: {distinct[:5]}'
    ...
    def _read_csv_chunks(self, path: Path, chunk_rows: int) -> Iterator[pd.DataFrame]:
        ...
        usecols=, pandas never classifies a line as bad - short rows are padded
        with NaN and long rows are truncated to the selected columns, so
    ...
    def _record_bad_lines(self, path: Path, bad_lines: List[List[str]]) -> None:
        ...
        Unreachable while the read kwargs pass usecols= (see _read_csv_chunks)
        - it exists so that a future kwargs change cannot reintroduce silent
        row loss.
        """
    ...
    def load_staging_file_group(self, files: List[str], table_name: str,
        ...
        # Notes are consolidated from the chunks this load already has in hand rather
        # than by a second pass over the extract, server-side or otherwise. The
        # measured saving is the point: folding 6.6M display lines into one note per
        # account costs ~120s of local CPU here, against ~290s of Azure SQL time for
        # the equivalent server-side statement - on an instance already contended by
        # other workloads. See notes_consolidation.NotesConsolidator.
        consolidator = (NotesConsolidator()
                        if table_name.upper() == NOTES_EXTRACT_TABLE else None)
        rows_read = 0
        rows_inserted = 0
        chunks = 0
        success = True
        ...
        # Deliberately last: every check above has to pass before the consolidated
        # notes are written. Consolidating a load that lost rows would produce account
        # entries that look complete and are not, and 67.tblentry_notes.sql has no way
        # to tell the difference.
        notes_accounts = 0
        if consolidator is not None and success:
            try:
                rolled = consolidator.result()
                notes_accounts = len(rolled)
                if notes_accounts:
                    rolled = rolled.rename(columns=NOTES_ACCOUNT_COLUMNS)
                    rolled['CreateTS'] = datetime.now()
                    if use_bcp:
                        ok, written = self.db_helper.bulk_insert_via_bcp(
                            rolled, NOTES_ACCOUNT_TABLE)
                    else:
                        ok, written = self.db_helper.bulk_insert_frame(
                            rolled, NOTES_ACCOUNT_TABLE)
                    if not ok or written != notes_accounts:
                        success = False
                        error_message = (
                            f'{NOTES_ACCOUNT_TABLE}: consolidated {notes_accounts} '
                            f'account(s) but wrote {written}')
                        self.logger.error(error_message)
                    else:
                        self.logger.info(
                            f'{NOTES_ACCOUNT_TABLE}: folded {rows_read} display '
                            f'line(s) into {notes_accounts} account note(s)')
            except Exception as e:
                # assert_lossless and the grouping guard both raise. Neither is
                # recoverable by retrying, and both mean tblEntry must not be fed.
                success = False
                error_message = f'notes consolidation failed: {e}'
                self.logger.error(error_message)
