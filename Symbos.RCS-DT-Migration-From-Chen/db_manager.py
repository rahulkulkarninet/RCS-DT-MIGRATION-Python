...
# Login timeout. The ODBC default is 15s, which is shorter than an Azure SQL
# login legitimately takes: a serverless database resuming from auto-pause, a
# failover moving the database to another node, and a first Entra token
# acquisition all routinely exceed it. Those all surfaced as
# "Login timeout expired (0) ... 08001" on the archive step - the last step of a
# run, after all of the migration work was already done.
DEFAULT_LOGIN_TIMEOUT = 60
# Connecting is retried like any other transient fault; 08001/"timeout" is
# already in the transient set below, it was just never given a retry path.
# More attempts than an ordinary statement gets, because a serverless database
# resuming refuses logins outright ("is not currently available, please retry")
# for the best part of a minute - measured at 53s and four attempts against
# test-migration-data. A permanent error, a wrong database name or a rejected
# credential, is not transient and still fails on the first attempt.
...
class DatabaseHelper:
    def __init__(self, environment: str , config_path: str = None):
        ...
        # Seconds to wait for a login before giving up on one attempt. Applied
        # to both the pyodbc and SQLAlchemy paths, which is why it lives here
        # rather than in the connection string - see connect_pyodbc.
        self.login_timeout = DEFAULT_LOGIN_TIMEOUT
        self.connect_attempts = DEFAULT_CONNECT_ATTEMPTS
        ...
        # sqlcmd timeouts. Idle-based, because file 78's loops legitimately run
    ...
    def connect_pyodbc(self) -> pyodbc.Connection:
        ...
        The login timeout is passed as pyodbc's `timeout` keyword rather than as
        a connection-string keyword, so the same value can be handed to
        SQLAlchemy through connect_args without depending on the driver
        accepting a particular spelling of it.
        """
        def attempt() -> pyodbc.Connection:
            conn_str = self._get_connection_string()
            connection = pyodbc.connect(conn_str, timeout=self.login_timeout)

            connection.setdecoding(pyodbc.SQL_CHAR, encoding='utf-8')
            connection.setdecoding(pyodbc.SQL_WCHAR, encoding='utf-8')
            connection.setencoding(encoding='utf-8')

            self.pyodbc_connection = connection
            self.logger.info(
        ...
        try:
            return self._run_with_retry(
                attempt,
                f'PyODBC connect to {self.config.get("DATABASE")} on '
    ...
    def connect_sqlalchemy(self) -> Engine:
        ...
        def attempt() -> Engine:
            ...
            engine = create_engine(
                ...
                pool_pre_ping=True,
                # Reaches pyodbc.connect as a keyword argument, so pooled
                # connections opened later carry the same login timeout as the
                # first one - the odbc_connect string cannot express it.
                connect_args={'timeout': self.login_timeout},
            )
        ...
        try:
            return self._run_with_retry(
                attempt,
                f'SQLAlchemy connect to {self.config.get("DATABASE")} on '
                f'{self.config.get("SERVER")}',
                max_attempts=self.connect_attempts,
            )
        except Exception as e:
    ...
    def execute_sqlcmd(self, sql_script: str, variables: Dict[str, Any] = None,
        ...
        try:
            ...
            if self.is_azure():
                # Same verified switches bcp uses, so file 78 cannot fail on
                ...
                # here means a uat run stops up front instead of failing at file 78
