...
### Cost code resolution and staging workflow

Unlike the mapping workflows above it does not rewrite extract values in place. It
resolves each cost code configured in variables/cost_codes.json to its tblCostType and
tblMasterCost row and rebuilds RC_STAGING_COSTS, which 51.tblcosts.sql then reads.
Because it writes a table of its own, a missing RC_STAGING_COSTS is treated as an
undeployed schema rather than bad data, and the run stops.

A charged code that cost_codes.json neither maps under `cost_types` nor excludes under
`not_costs` fails the customer instead of migrating: an INNER JOIN used to drop those
silently, which is how $403,234.13 of COL charges reached an empty tblCost.

CustomerProcessor wrappers delegate to CustomerCostWorkflowService:

- update_costs -> update_costs
- _check_costs_step -> check_costs_step

Files:
- customer_processor.py
- services/workflows/customer_cost_workflow_service.py

### Operator contact normalization workflow

A multi-table in-place rewrite, like the Payment_Method one, not a staging table like
costs. It rewrites the operator column of each extract - RC_ACCOUNT_EXTRACT.Operator,
RC_PAYMENTS.Payment_Operator, RC_SMS.Operator_Code, RC_EMAIL_EXTRACT.Op_code,
RC_DOCHIST_EXTRACT.Operator_Code and RC_NOTES_EXTRACT.Operator - to the canonical
tblContact.UserName named in variables/operator_contact_codes.json. Seven files then
read tblContact directly with OUTER APPLY: 01.tblaccount, 49.tblpayment,
58.tblsmsoutput, 60.tblemail, 64.tbldocument, 65.tblletter and 67.tblentry_notes,
replacing the customer-specific CSRC_OperatorContactMapping they used to join by name.
Nothing is deployed and no table is created.

An operator code the file does not list is not a failure: the SQL ends every operator
lookup with ISNULL(OM.ContactID, {{DefaultOperatorContactID}}), so the row is attributed
to the fallback contact (PRAM, resolved per environment from migration_variables.json),
and a code that is already a username resolves with no entry at all. The check reports
unresolved codes busiest first and the customer continues, which is what lets the file be
added to as needed rather than maintained as a complete inventory. Two things are fatal -
a username the file names that tblContact does not have, because its codes would fall
through to the fallback silently and a typo would be indistinguishable from a working
mapping, and an
operator column too narrow for a configured username, which is pre-flighted the way
FrequencyService pre-flights RC_ARRANGEMENT.Frequency.

CustomerProcessor wrappers delegate to CustomerOperatorContactWorkflowService:

- update_operator_contacts -> update_operator_contacts
- _check_operator_contacts_step -> check_operator_contacts_step

Files:
- customer_processor.py
- services/workflows/customer_operator_contact_workflow_service.py
...
- CostService for the RC_COSTS_EXTRACT unpivot and RC_STAGING_COSTS rebuild.
- OperatorContactService for low-level operator code normalization and DB update primitives.
- text_normalization for the lookup-key rules shared by all the mapping services.
